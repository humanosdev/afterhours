/**
 * Runs in <head> during development only, before Next.js chunk scripts execute.
 *
 * 1. Always unregisters service workers + clears Cache Storage (no long-lived skip —
 *    the old `ah-dev-purge-v10` skip caused stale precache to keep serving HTML that
 *    pointed at deleted chunk URLs → 404 on `/_next/static/chunks/app/...` and a blank page).
 * 2. Auto-reloads at most once per tab when we actually removed an SW, caches, or had a
 *    controlling worker, so we do not loop.
 * 3. After DOM is ready (and again on load + delayed pass), probes script + stylesheet
 *    URLs under `/_next/static/`; if any 404, purge and reload up to MAX_CHUNK_FIX times
 *    so one failed recovery does not strand the tab.
 */
export const DEV_PURGE_SERVICE_WORKER_SCRIPT = `
(function () {
  try {
    var RELOAD_KEY = "ah-dev-purge-reloaded-v13";
    var CHUNK_ATTEMPTS_KEY = "ah-dev-chunk-fix-attempts-v2";
    var MAX_CHUNK_FIX = 5;
    var hadRegs = false;
    var hadCaches = false;
    var ctrl = !!(navigator.serviceWorker && navigator.serviceWorker.controller);
    var chunkProbeInflight = false;

    function purgeAll() {
      var p = Promise.resolve();
      if ("serviceWorker" in navigator) {
        p = navigator.serviceWorker.getRegistrations().then(function (regs) {
          hadRegs = regs.length > 0;
          return Promise.all(regs.map(function (r) { return r.unregister(); }));
        });
      }
      return p.then(function () {
        if (!("caches" in window)) return;
        return caches.keys().then(function (keys) {
          hadCaches = keys.length > 0;
          return Promise.all(keys.map(function (k) { return caches.delete(k); }));
        });
      });
    }

    function maybeReloadAfterPurge() {
      if (!(hadRegs || hadCaches || ctrl)) return;
      if (sessionStorage.getItem(RELOAD_KEY) === "1") return;
      sessionStorage.setItem(RELOAD_KEY, "1");
      try {
        sessionStorage.removeItem("ah-dev-purge-v10");
      } catch (e) {}
      location.reload();
    }

    purgeAll()
      .then(maybeReloadAfterPurge)
      .catch(function () {});

    function collectStaticUrls() {
      var urls = [];
      var nodes = document.querySelectorAll('script[src*="/_next/static/"]');
      for (var i = 0; i < nodes.length && urls.length < 8; i++) {
        var s = nodes[i] && nodes[i].src;
        if (s) urls.push(String(s));
      }
      var links = document.querySelectorAll('link[rel="stylesheet"][href*="/_next/static/"]');
      for (var j = 0; j < links.length && urls.length < 10; j++) {
        var h = links[j] && links[j].href;
        if (h) urls.push(String(h));
      }
      return urls;
    }

    function runChunkProbe() {
      if (chunkProbeInflight) return;
      var urls = collectStaticUrls();
      if (!urls.length) return;
      chunkProbeInflight = true;
      Promise.all(
        urls.map(function (u) {
          return fetch(u, { method: "GET", cache: "no-store" })
            .then(function (r) { return !!(r && r.ok); })
            .catch(function () { return false; });
        })
      )
        .then(function (flags) {
          chunkProbeInflight = false;
          var any404 = flags.some(function (ok) { return !ok; });
          if (!any404) {
            try {
              sessionStorage.removeItem(CHUNK_ATTEMPTS_KEY);
            } catch (e) {}
            return;
          }
          var n = parseInt(sessionStorage.getItem(CHUNK_ATTEMPTS_KEY) || "0", 10) || 0;
          if (n >= MAX_CHUNK_FIX) return;
          sessionStorage.setItem(CHUNK_ATTEMPTS_KEY, String(n + 1));
          sessionStorage.removeItem(RELOAD_KEY);
          hadRegs = false;
          hadCaches = false;
          purgeAll().finally(function () {
            location.reload();
          });
        })
        .catch(function () {
          chunkProbeInflight = false;
        });
    }

    [0, 30, 120, 400, 900, 2000].forEach(function (ms) {
      setTimeout(runChunkProbe, ms);
    });
    document.addEventListener("DOMContentLoaded", function () {
      [0, 100, 400, 1200].forEach(function (ms) {
        setTimeout(runChunkProbe, ms);
      });
    });
    window.addEventListener("load", function () {
      setTimeout(runChunkProbe, 80);
    });
  } catch (e) {}
})();
`.trim();

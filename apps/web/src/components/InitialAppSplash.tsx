...
/** Hub must dispatch `ah-hub-feed-ready` (or we bail) so splash never blocks the shell indefinitely. */
const HUB_WAIT_MAX_MS = 8000;
const MAP_WAIT_MAX_MS = 10_000;
const FADE_MS = 240;

type Phase = "off" | "on" | "out";

/**
 * Once per browser tab: full-screen primary canvas (`--ah-bg-primary`) first for everyone.
...
export default function InitialAppSplash({ isAuthed }: { isAuthed: boolean }) {
  ...
  useEffect(() => {
    ...
    const waitHubIfNeeded = () =>
      new Promise<void>((resolve) => {
        ...
        const finish = () => {
          ...
          window.removeEventListener("ah-hub-feed-ready", onReady);
          window.clearInterval(pathPoll);
          window.clearTimeout(maxWait);
          resolve();
        };
        const onReady = () => finish();
        const maxWait = window.setTimeout(finish, HUB_WAIT_MAX_MS);
        window.addEventListener("ah-hub-feed-ready", onReady, { once: true });
        const pathPoll = window.setInterval(() => {
          if (pathRef.current !== "/hub" || !authedRef.current) finish();
        }, 250);
      });
    ...
    const waitMapIfNeeded = () =>
      new Promise<void>((resolve) => {
        ...
        const finish = () => {
          ...
          window.removeEventListener("ah-map-ready", onReady);
          window.clearInterval(pathPoll);
          window.clearTimeout(maxWait);
          resolve();
        };
        const onReady = () => finish();
        const maxWait = window.setTimeout(finish, MAP_WAIT_MAX_MS);
        window.addEventListener("ah-map-ready", onReady, { once: true });
        const pathPoll = window.setInterval(() => {
          if (pathRef.current !== "/map" || !authedRef.current) finish();
        }, 250);
      });
    ...
    void (async () => {
      ...
      console.warn("[InitialAppSplash] splash sequence failed:", err);
      try {
        sessionStorage.setItem(AH_SPLASH_SESSION_KEY, "1");
      } catch {
        /* private mode */
  ...
  const inner = (
    <div
      className={`flex min-h-[100dvh] w-full items-center justify-center bg-primary px-4 transition-opacity duration-[280ms] ease-out ${

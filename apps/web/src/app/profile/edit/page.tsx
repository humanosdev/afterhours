...
function fileLooksLikeImage(file: File): boolean {
  ...
  if (AVATAR_EXTENSION_FALLBACKS.some((ext) => name.endsWith(ext))) return true;
  /** iOS / Android sometimes send empty `type` for camera-roll picks — still try decode. */
  if (!type && file.size > 0 && file.size <= 25 * 1024 * 1024) return true;
  return false;
}
...
const uploadAvatar = async (file: File) => {
  ...
  if (uploadError) {
    const msg = (uploadError.message || "").trim();
    const lower = msg.toLowerCase();
    if (lower.includes("row-level security") || lower.includes("rls") || lower.includes("policy")) {
      setError(
        "Upload was blocked by storage permissions. In Supabase: run the migration that creates the `avatars` bucket and policies (see `zzz_storage_avatars_bucket.sql`), or add INSERT/UPDATE policies for `storage.objects`."
      );
    } else if (lower.includes("bucket") && lower.includes("not found")) {
      setError(
        "The avatars storage bucket is missing. Create a public bucket named `avatars` in Supabase Storage, then apply the project migration for avatar policies."
      );
    } else if (lower.includes("size") || lower.includes("too large") || lower.includes("entity too large")) {
      setError("Image is too large for storage. Try a smaller photo or crop tighter.");
    } else if (msg) {
      setError(`Could not upload: ${msg.length > 220 ? `${msg.slice(0, 220)}…` : msg}`);
    } else {
      setError("Could not upload profile picture. Check the browser console for details.");
    }
    console.warn("Avatar storage upload failed:", uploadError);
...
const validateAvatarFile = (file: File) => {
  ...
  if (!fileLooksLikeImage(file)) {
    setError("Please choose an image file from your camera roll or files app.");
    return false;
  }
  if (file.size > maxBytes) {
...
{cropSrc && (
  <div className="fixed inset-0 z-[10170] bg-primary/90">
...
<div className="mb-6 flex flex-col items-center">
  <label className="cursor-pointer">
    ...
    <input
      type="file"
      accept="image/*,.heic,.heif,.avif,.jfif,.dng,.raw,.cr2,.nef"
      className="hidden"
      onChange={(e) => {
        if (e.target.files?.[0]) {
    ...
    Most camera-roll images supported · we optimize to lightweight JPG automatically
      ...
      <div className="mb-5">
        ...
        <input
          ...
          maxLength={20}
          className="w-full rounded-xl bg-primary/40 border border-white/10 p-3 outline-none"
        />

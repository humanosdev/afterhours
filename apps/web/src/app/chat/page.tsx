...
export default function ChatPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // -------- Load friends (accepted, excluding any active block with me) --------
      let friendIds: string[];
      try {
        friendIds = await acceptedFriendIdsExcludingBlocks(supabase, user.id);
      } catch {
        setFriends([]);
        setLoading(false);
        return;
  ...
  useEffect(() => {
    ...
    const bump = () => void reloadFriendsOnly();
  ...
  return (
    <ProtectedRoute>
    <div className={`${APP_TAB_PAGE_ROOT_CLASS} h-full overflow-hidden text-white`}>
      <div className={`${APP_CONTENT_MAX_CLASS} flex h-full min-h-0 w-full flex-1 flex-col overflow-hidden`}>
      <div className="sticky top-0 z-20 shrink-0 border-b border-white/[0.08] bg-primary/92 px-4 pb-3 pt-[calc(env(safe-area-inset-top,0px)+10px)] backdrop-blur-xl">
        <div className="flex min-h-[44px] items-center justify-between">
          {selectionMode ? (
            <>
              <button
                onClick={exitSelectionMode}
                className="ah-glass-control ah-glass-control-interactive rounded-full px-3 py-1 text-sm text-white/80"
              >
                <span>Cancel</span>
              </button>
            ...
            <>
              ...
              <button
                ...
                className="ah-glass-control ah-glass-control-interactive rounded-full px-3 py-1.5 text-[13px] font-semibold text-white/85"
              >
                <span>New</span>
              </button>
            </>
      ...
      <div
        className={`min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-3 pt-1 sm:px-4 ${APP_PAGE_TAIL_PADDING_CLASS}`}
      >
        {loading ? (

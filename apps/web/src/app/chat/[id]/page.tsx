...
export default function ChatConversationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (prof && meId) {
        const status = await getPairBlockStatus(supabase, meId, otherId);
        if (!cancelled) setPairBlock(status);
      } else if (!cancelled) {
        setPairBlock("none");
      }
      finishGate();
  ...
  return (
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-primary text-white">
      ...
      <div className="sticky top-0 z-20 border-b border-white/[0.08] bg-primary/92 px-3 pb-2.5 pt-[calc(env(safe-area-inset-top,0px)+8px)] backdrop-blur-xl sm:px-4">
        <div className="flex min-h-[44px] items-center gap-2">
          <SubpageBackButton onBack={() => navigateBack(router, "/chat")} />
              ...
              {activeMessageMenuId === m.id ? (
                ...
                >
                  ...
                  <button
                    ...
                    className="rounded-lg border border-white/15 bg-primary/60 px-2 py-1 text-xs text-white/80"
                  >
                    Delete for me
                  </button>
                </div>
      ...
      <div
        className="border-t border-white/10 bg-primary/90 px-3 py-2 backdrop-blur"
        style={{ paddingBottom: `calc(env(safe-area-inset-bottom, 0px) + ${composerBottomInset}px)` }}
      >
        {pairBlock !== "none" ? (
          <p className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-center text-[13px] leading-snug text-white/65">

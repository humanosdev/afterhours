...
import { SubpageBackButton } from "@/components/AppSubpageHeader";
...
function LoginPageContent() {
  ...
  const accountBanner = (() => {
    ...
    if (raw === "removed") {
      return "This session is no longer linked to an account. Sign in again if you have another profile.";
    }
    if (raw === "paused") {
      return "Your account is paused. Sign back in anytime to resume.";
    }
    if (raw === "deleted") {
      return "Deletion has been requested. You can sign back in before the scheduled removal date to cancel it in Settings.";
    }
    return null;
  })();
  ...
  return (
    <AuthScreenShell marketing className="transition-colors">
      <div className="mb-4 flex w-full justify-start">
        <SubpageBackButton onBack={() => router.push("/")} />
      </div>
      ...
      <form onSubmit={onLogin} className="mt-8 w-full space-y-4">
        ...
        {accountBanner ? (
          <div className="rounded-xl border border-white/15 bg-white/[0.06] p-3 text-sm text-white/75">{accountBanner}</div>
        ) : null}

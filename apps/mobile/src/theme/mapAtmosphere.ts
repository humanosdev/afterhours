...
/** Mapbox fog `star-intensity` — world-anchored procedural stars, eases toward a street-level floor. */
...
/** PWA `applyMapAtmosphereForMode` — Mapbox `Atmosphere` / fog props (globe space stars). */
export function mapAtmosphereStyleForDayMode(dayMode: boolean, zoom = 14) {
  ...
  if (dayMode) {
    return {
      ...
      color: "#eef0f4",
      highColor: inSpace ? "#6b8fc7" : "#e5e8ef",
      horizonBlend: horizonBlendForZoom(zoom, true),
      /** Deep blue-black space — Mapbox procedural stars read slightly cool against this. */
      spaceColor: "#050814",
      starIntensity,
    };
  }
  ...
  return {
    ...
    color: "#0c1118",
    highColor: inSpace ? "#1e4080" : "#131b26",
    horizonBlend: horizonBlendForZoom(zoom, false),
    spaceColor: "#050814",
    starIntensity,
  };
}
